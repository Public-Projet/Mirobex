    ...
    <nav class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between h-16">
        ...
        <div class="hidden sm:ml-6 sm:flex sm:space-x-8">
          <a v-for="(link, index) in NavLinks" :key="index" :href="link.to" class="nav-link">
            {{ link.text }}
          </a>
        ...
        <div class="flex items-center space-x-4">
          <button @click="openContactModal"
            class="hidden sm:block bg-primary text-white px-4 py-2 rounded-md hover:bg-secondary hover:text-black transition-colors text-base">
            Nous contacter
          </button>
          ...
          <button @click="isMenuOpen = !isMenuOpen"
            ...
            <IconMenu class="w-auto h-[1em] scale-[1.5] text-primary"
              :class="{ 'hidden': isMenuOpen, 'block': !isMenuOpen }" />
            <IconX class="w-auto h-[1em] scale-[1.5] text-primary"
      ...
      <div class="sm:hidden transition-all duration-300 ease-in-out"
        ...
        <div class="py-3 space-y-1">
          <a v-for="(link, index) in NavLinks" :key="index" :href="link.to"
          ...
          <NuxtLink to="/terms"
            class="block px-4 py-2 text-base font-medium text-gray-700 hover:text-primary hover:bg-gray-50"
            @click="isMenuOpen = false">
            Termes et Références
          </NuxtLink>
          <button @click="openContactModal"
            class="block w-full text-left bg-primary text-white px-4 py-2 rounded-md hover:bg-secondary hover:text-black transition-colors text-base mt-2">
            Nous contacter
          </button>
        </div>
      </div>
        ...
        <div class="space-y-4">
          ...
          <div class="space-y-3">
            <a href="tel:+2290147797912"
              class="flex items-center space-x-2 p-3 border rounded-lg hover:bg-gray-50 transition-colors">
